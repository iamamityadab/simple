...
try:
  ...
  if form.getvalue("b1")=="Update":
    con=mysql.connector.connect(user='root',password='',host='localhost', database='hospital')
    cur=con.cursor()
    cur.execute("update permission set roll_id='"+t2+"',title='"+t3+"',module='"+t4+"',descr='"+t5+"' where id='"+t1+"'")
    con.commit()
    print("<script>alert('Record Updated....')</script>")
  if form.getvalue("b1")=="Particular Search":
    con=mysql.connector.connect(user='root',password='',host='localhost', database='hospital')
    print("<table border=1><tr><th>ID</th><th>Roll ID</th><th>Title</th><th>Module</th><th>Description</th></tr>")
    cur=con.cursor()
    cur.execute("select * from permission where id='"+t1+"'")
    r=cur.fetchall()
    for x in r:
      print("<form name='f' method='post' action='permissionUpdate.py'>")
      print("<tr>")
