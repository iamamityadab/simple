...
try:
  if form.getvalue("b1")=="Save":
    ...
    if(f==1):
      ...
      cur.execute("insert into doctor values('"+t1+"','"+t2+"','"+t3+"','"+t4+"','"+t5+"','"+t6+"','"+t7+"')")
      con.commit()
      print("<script>alert('Record Saved....')</script>")
  if form.getvalue("b1")=="Delete":
    con=mysql.connector.connect(user='root',password='',host='localhost', database='hospital')
    cur=con.cursor()
    cur.execute("delete from doctor where id='"+t1+"'")
    con.commit()
    print("<script>alert('Record Deleted....')</script>")
  
  if form.getvalue("b1")=="Particular Search":
    con=mysql.connector.connect(user='root',password='',host='localhost', database='hospital')
    print("<table border=1><tr><th>Doctor ID</th><th>Name</th><th>Mobile</th><th>Email</th><th>Address</th><th>Password</th><th>User Name</th></tr>")
    cur=con.cursor()
    cur.execute("select * from doctor where id='"+t1+"'")
    r=cur.fetchall()
    for x in r:
      print("<form name='f' method='post' action='doctorUpdate.py'>")
      print("<tr>")
  ...
  if form.getvalue("b1")=="Update":
    con=mysql.connector.connect(user='root',password='',host='localhost', database='hospital')
    cur=con.cursor()
    cur.execute("update doctor set name='"+t2+"',mobile='"+t3+"',email='"+t4+"',address='"+t5+"',password='"+t6+"',username='"+t7+"' where id='"+t1+"'")
    con.commit()
